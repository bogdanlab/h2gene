...
parser <- add_option(parser, '--gene_list', type = 'character', help="list of genes to consider")
parser <- add_option(parser, '--sumstats', type = 'character')

# options
parser <- add_option(parser, '--num_samples', type = 'integer', default=500, help = "Number of posterior samples")
parser <- add_option(parser, '--window_size', type='integer', default=500, help="Window size to consider (in kilo base)")
parser <- add_option(parser, '--min_cor', type='double', default=0.2, help="threshold of SNPs")
parser <- add_option(parser, '--tss_window', type='integer', default=10, help="tss window size around the gene body to consider (in kilobases)")
...
read_data <- function(parser){
    ...
    # snp_info
    sumstats <- read_tsv(parser$sumstats, col_types=cols(CHR="i", SNP="c", BP="i", A1="c", A2="c", N="i", MAF="d", LDSCORE="d", Z="d"))
    ...
    ld <- (ld + t(ld))/2
    ld_snps <- read_table(paste0(parser$ld_prefix, '.snps'), col_types='c', col_names='SNP')
...
MAF_RANGE <- tibble(LEVEL=c('RARE', 'LF', 'COMMON'),
                    START=c(0.005, 0.01, 0.05),
                    STOP=c(0.01, 0.05, 0.5))

add_annotation <- function(sumstats, gene, tss_window){
    sumstats$ANNOT_BODY_ALL <- ((gene$START <= sumstats$BP) & (sumstats$BP < gene$STOP))
    sumstats$ANNOT_TSS_ALL <- ((gene$START - tss_window * 1000 <= sumstats$BP) & (sumstats$BP < gene$START)) |
                              ((gene$STOP <= sumstats$BP) & (sumstats$BP < gene$STOP + tss_window * 1000))
    sumstats$ANNOT_BODYTSS_ALL <- ((gene$START - tss_window * 1000 <= sumstats$BP) & 
                                   (sumstats$BP < gene$STOP + tss_window * 1000))
    
    for (level_i in 1 : nrow(MAF_RANGE)){
        maf_range <- MAF_RANGE[level_i, ]
        maf_range_snps <- (maf_range$START <= sumstats$MAF) & (sumstats$MAF < maf_range$STOP)
        sumstats[[paste0('ANNOT_BODYTSS_', maf_range$LEVEL)]] <- (sumstats$ANNOT_BODYTSS_ALL & maf_range_snps)
    }
    return(sumstats)
}
if ( nrow(gene_list) > 0 & (!is.null(ld)) ){
    ...
    for (gene_i in 1 : nrow(gene_list)){
        ...
        for (annot_i in 1 : ncol(annot_cols)){
            ...
            gene[[paste0(annot, '_AVG_LDSCORE')]] <- mean(gene_sumstats$LDSCORE[annot_cols[[annot]]])
            gene[[paste0(annot, '_AVG_MAF')]] <- mean(gene_sumstats$MAF[annot_cols[[annot]]])
        }

        # get a `window_size` window
        window_snps <- ((gene$START - parser$window_size * 1000 <= gene_sumstats$BP) & 
                        (gene_sumstats$BP < gene$STOP + parser$window_size * 1000))
        cor_snps <- apply(abs(ld[gene_sumstats$ANNOT_BODYTSS_ALL, , drop=FALSE] > parser$min_cor), 2, any)
        ...
        estimate_list[[gene_i]] <- list(gene=gene, 
