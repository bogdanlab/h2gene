...
parser <- add_option(parser, "--snp_info", type = "character")
...
parser <- add_option(parser, "--PI_prob",
    action = "store", type = "double", default = 0.9
)
...
snp_info <- read_tsv(parser$snp_info,
    col_types = cols(
        CHR = "i",
        SNP = "c",
        CM = "i",
...
pb <- progress_bar$new(total = nrow(partition))
file_missing <- FALSE
...
for (par_i in 1:nrow(partition)) {
    ...
    par <- partition[par_i, ]
    print("here")
    par_ld <- np$load(file.path(
        parser$ld_dir, paste0("par_", par_i - 1, ".npy")
    ))
    ...
    par_gene_list <- gene_list[
        (gene_list$START >= par$START) & (gene_list$STOP < par$STOP),
    ]
    ...
    if (nrow(par_gene_list) > 0) {
        for (sim_i in 1:n_sim) {
            rds_file <- file.path(
                estimate_dir,
                paste0("sim_", sim_i - 1, "_par_", par_i - 1, ".rds")
            )
            ...
            for (gene_i in 1:nrow(par_gene_list)) {
                ...
                gene_annot <- par_estimate[[gene_i]]$gene_annot
                gene_results <- tibble(NAME = gene$NAME, SIM_I = sim_i)

                for (maf_bin in MAF_BINS) {
                    annot_name <- paste0("BODYTSS_", maf_bin)
                    annot_snps <- gene_annot[[annot_name]]
                    maf_bin_ld <- par_ld[annot_snps, annot_snps, drop = FALSE]
                    maf_bin_beta <- par_beta[annot_snps, sim_i]
                    # ground truth
                    gene_results[[paste0(maf_bin, "_TRUE_VAR")]] <- as.numeric(
                        (maf_bin_beta %*% maf_bin_ld) %*% maf_bin_beta
                    )
                    # h2 estimates
                    gene_results[[paste0(maf_bin, "_MEAN")]] <- mean(
                        h2_estimates[[annot_name]]
                    )
                    gene_results[[paste0(maf_bin, "_SD")]] <- sd(
                        h2_estimates[[annot_name]]
                    )
                    gene_results[[paste0(maf_bin, "_LOWER")]] <- quantile(
                        h2_estimates[[annot_name]], (1 - parser$PI_prob) / 2
                    )
                    gene_results[[paste0(maf_bin, "_UPPER")]] <- quantile(
                        h2_estimates[[annot_name]], 1 - (1 - parser$PI_prob) / 2
                    )
                    # causal number estimates
                    gene_results[[paste0(maf_bin, "_CAUSAL_NUM_MEAN")]] <- mean(
                        causal_num_estimates[[annot_name]]
                    )
                    gene_results[[paste0(maf_bin, "_CAUSAL_NUM_SD")]] <- sd(
                        causal_num_estimates[[annot_name]]
                    )
...
for (maf_bin_i in 1:length(MAF_BINS)) {
    ...
    p1 <- ggplot(estimates, aes(x = TRUE_VAR, y = MEAN)) +
        ...
        labs(title = maf_bin) +
        annotate(
            geom = "text", label = lm_label,
            x = -Inf, y = Inf, hjust = 0, vjust = 1, parse = TRUE
        )
